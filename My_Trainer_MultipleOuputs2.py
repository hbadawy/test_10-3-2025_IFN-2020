...
class change_detection_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # label2 = F.interpolate(label_tensor, scale_factor=(1/2), mode='bilinear', align_corners=False)
        # label3 = F.interpolate(label_tensor, scale_factor=(1/4), mode='bilinear', align_corners=False)
        # label4 = F.interpolate(label_tensor, scale_factor=(1/8), mode='bilinear', align_corners=False)
        # label5 = F.interpolate(label_tensor, scale_factor=(1/16), mode='bilinear', align_corners=False)    
        
        # return {'pre':pre_tensor,'post':post_tensor,'label1':label_tensor,'label2':label2,'label3':label3,'label4':label4 ,'label5':label5  ,'fname':fname}
        return {'pre':pre_tensor,'post':post_tensor,'label1':label_tensor,'fname':fname}
...
def train(model, train_loader, val_loader, optimizer, loss_function1,loss_function2,loss_function3,loss_function4,loss_function5, device, num_epochs, save_path, scheduler):
    ...
    for epoch in range(num_epochs):
        ...
        for _, data in enumerate(train_loader):
            optimizer.zero_grad()
            pre_tensor, post_tensor, label_tensor, fname = data["pre"], data["post"], data["label1"], data["fname"]
            ...
            label2 = F.interpolate(label_tensor, scale_factor=(1/2), mode='bilinear', align_corners=False).to(device)
            label3 = F.interpolate(label_tensor, scale_factor=(1/4), mode='bilinear', align_corners=False).to(device)
            label4 = F.interpolate(label_tensor, scale_factor=(1/8), mode='bilinear', align_corners=False).to(device)
            label5 = F.interpolate(label_tensor, scale_factor=(1/16), mode='bilinear', align_corners=False).to(device)
            ...
            total_loss1=loss_function1(prediction[0],label_tensor)
            total_loss2=loss_function2(prediction[1],label2)
            total_loss3=loss_function3(prediction[2],label3)
            total_loss4=loss_function4(prediction[3],label4)
            total_loss5=loss_function5(prediction[4],label5)
            total_loss = (total_loss1 + total_loss2 + total_loss3 + total_loss4 + total_loss5)/5
        ...
        if (epoch+1) % num_val_epochs ==0:
            ...
            for _, data in enumerate(val_loader):
                ...
                label2 = F.interpolate(label_tensor, scale_factor=(1/2), mode='bilinear', align_corners=False).to(device).to(device)
                label3 = F.interpolate(label_tensor, scale_factor=(1/4), mode='bilinear', align_corners=False).to(device).to(device)
                label4 = F.interpolate(label_tensor, scale_factor=(1/8), mode='bilinear', align_corners=False).to(device).to(device)
                label5 = F.interpolate(label_tensor, scale_factor=(1/16), mode='bilinear', align_corners=False).to(device).to(device)
                ...
                val_loss1=loss_function1(probs[0],label_tensor)
                val_loss2=loss_function2(probs[1],label2)
                val_loss3=loss_function3(probs[2],label3)
                val_loss4=loss_function4(probs[3],label4)
                val_loss5=loss_function5(probs[4],label5)
                val_loss = (val_loss1 + val_loss2 + val_loss3 + val_loss4 + val_loss5)/5 

                # val_loss=loss_function(probs,label_tensor)   #added new
                # scheduler.step(val_loss)
                val_list.append(val_loss.item())     #added new

                prediction = torch.where(probs[0]>0.5,1,0)
...
if __name__ == "__main__":
    ...
    loss_function1 = cd_loss()
    loss_function2 = cd_loss()
    loss_function3 = cd_loss()
    loss_function4 = cd_loss()
    loss_function5 = cd_loss()

    num_epochs = 100
    save_path = "E://VS Projects//test_10-3-2025_IFN-2020//Checkpoints//checkpoint5-BCE-100epochs-"
    os.makedirs(save_path, exist_ok=True)

    loss_gr_list, loss_gr_list_val = train(model, train_loader=train_loader, val_loader=val_loader, 
                                              optimizer=optimizer, loss_function1=loss_function1,loss_function2=loss_function2,loss_function3=loss_function3, 
                                              loss_function4=loss_function4, loss_function5=loss_function5, device=device, 
