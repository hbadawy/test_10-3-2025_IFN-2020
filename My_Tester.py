...
from My_Trainer_MultipleOuputs2 import test_loader
import time
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("E://VS Projects//test_10-3-2025_IFN-2020//Checkpoints//checkpoint5-BCE-100epochs-//ResUnet99.pth"))
    model.eval()
    test_results_path="E://VS Projects//test_10-3-2025_IFN-2020//test_results5-BCE-100epochs"
    ...
    for _, data in enumerate(test_loader):
        ...
        # label2 = F.interpolate(label_tensor, scale_factor=(1/16), mode='bilinear', align_corners=False).to(device).to(device)

        probs = model(pre_tensor, post_tensor)

        prediction = torch.where(probs[0]>0.5,1.0,0.0)
    ...
    pre_tensor, post_tensor, label_tensor, fname = data["pre"], data["post"], data["label1"], data["fname"]
    # label_small = F.interpolate(label_tensor, scale_factor=(1/16), mode='bilinear', align_corners=False).cpu()
